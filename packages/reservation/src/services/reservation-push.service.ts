...
export class ReservationPushService {
  ...
  private messagesBody = {
    [ReservationPushTypes.before24hours]: (dateTime, clinicName) =>
      `You have a Covid-19 test scheduled for ${getDateDefaultHumanReadable(
        dateTime,
      )} at our ${clinicName} location.`,
    [ReservationPushTypes.before3hours]: (dateTime, clinicName) =>
      `Your Covid-19 test is scheduled for today at ${getDateDefaultHumanReadable(
        dateTime,
      )} at our ${clinicName} location.`,
