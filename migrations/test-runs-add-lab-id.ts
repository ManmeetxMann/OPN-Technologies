...
async function updateTestRuns(): Promise<Result[]> {
  let offset = 0
  let hasMore = true
  ...
  while (hasMore) {
    const testRunsSnapshot = await database
      .collection('test-runs')
      .offset(offset)
      .limit(limit)
      .get()

    offset += testRunsSnapshot.docs.length
    hasMore = !testRunsSnapshot.empty
    //hasMore = false

    for (const testRun of testRunsSnapshot.docs) {
      const promises = []
      promises.push(addLabId(testRun))
...
async function addLabId(snapshot: firestore.QueryDocumentSnapshot<firestore.DocumentData>) {
  const updateData = async (record) => {
    ...
    if (!isEmpty(data)) {
      await record.ref.set(
        {
          ...
          timestamps: {
            migrations: {
              testRunsAddLabId: firestore.FieldValue.serverTimestamp(),
            },
          },
        },
        {
      ...
      console.log(`Successfully updated Test Runs: ${record.id}`)
      return 'Updated'
    }
    return
  }
...
async function main() {
  try {
    ...
    const results = await updateTestRuns()
